...
void main() {
  group('validateVoiceNoteAudio', () {
    ...
    test('rejects audio larger than kMaxVoiceNoteBytes', () {
      expect(
        () => validateVoiceNoteAudio(List.filled(kMaxVoiceNoteBytes + 1, 0), 5),
  ...
  group('VoiceNoteService', () {
    ...
    test(
      'send rejects an oversized note before touching the network',
      () async {
    ...
    test(
      'send delivers a base64-encoded note, watchVoiceNotes decodes it',
      () async {
        final sockets = <String, FakeRelaySocket>{};
        final pool = RelayPool([
          'wss://a',
