...
void main() {
  ...
  test(
    "subscribes on the driver's own geohash cell plus its 8 neighbors",
    () async {
  ...
  test('yields a parsed, unexpired ride request', () async {
    ...
    expect(
      got.first.request.pickupGeohash,
      geohashEncode(driverLat, driverLon, precision: 6),
    );
    await sub.cancel();
  });

  test("drops a request whose NIP-40 expiration has already passed", () async {
    final sockets = <String, FakeRelaySocket>{};
    final pool = RelayPool([
      'wss://a',
    ], connect: (u) => sockets[u] = FakeRelaySocket());
